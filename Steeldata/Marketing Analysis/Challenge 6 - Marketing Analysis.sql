...
SELECT campaign_id, campaign_name, COUNT(*) AS total_transactions FROM (
SELECT m.campaign_id, m.campaign_name
FROM marketing_campaigns m
JOIN transactions t
ON t.purchase_date
BETWEEN m.start_date AND m.end_date) tt
GROUP BY campaign_id, campaign_name;

--2. Which product had the highest sales quantity?

SELECT TOP 1 product_id, product_name, SUM(quantity) AS total_quantity_sold FROM (
SELECT sc.product_id, sc.product_name, t.quantity
FROM sustainable_clothing sc
JOIN transactions t
ON sc.product_id = t.product_id) tt
GROUP BY product_id, product_name
ORDER BY total_quantity_sold DESC;

--3. What is the total revenue generated from each marketing campaign?

SELECT campaign_id, campaign_name, SUM(total_price) AS total_sales FROM (
SELECT m.campaign_id, m.campaign_name, (t.quantity*sc.price) AS total_price
FROM transactions t
JOIN sustainable_clothing sc
ON sc.product_id = t.product_id
JOIN marketing_campaigns m
ON t.purchase_date BETWEEN m.start_date AND m.end_date) tt
GROUP BY campaign_id, campaign_name

--4. What is the top-selling product category based on the total revenue generated?

SELECT Top 1 category, SUM(total_price) AS total_sales FROM (
SELECT sc.category, (t.quantity*sc.price) AS total_price
FROM sustainable_clothing sc
JOIN transactions t
ON t.product_id = sc.product_id) tt
GROUP BY category
...
SELECT ROUND(AVG(t.quantity*sc.price),2) AS avg_rev_per_day
FROM transactions t
JOIN sustainable_clothing sc
ON sc.product_id = t.product_id
JOIN marketing_campaigns m
ON sc.product_id = m.product_id
...
WITH cte1 AS (
SELECT sc.product_name, (sc.price * t.quantity) AS total_sales
FROM sustainable_clothing sc
JOIN transactions t
ON sc.product_id = t.product_id)
, cte2 AS (SELECT product_name, SUM(total_sales) AS ts, (SELECT ROUND(SUM(total_sales),2) FROM cte1) tt
FROM cte1
GROUP BY product_name)
SELECT product_name, ROUND((1.0*ts/tt*100),2) AS total_sales_contribution
FROM cte2
ORDER BY total_sales_contribution DESC

--8. Compare the average quantity sold during marketing campaigns to outside the marketing campaigns

WITH quantity_during_campaign AS (
SELECT AVG(t.quantity) AS avg_quantity_campaign
FROM transactions t
JOIN marketing_campaigns mc
ON t.product_id = mc.product_id),

quantity_during_non_campaign AS (
SELECT AVG(t.quantity) AS avg_non_quantity_campaign
FROM transactions t
LEFT JOIN marketing_campaigns mc
ON t.product_id = mc.product_id
WHERE mc.product_id IS NULL)

SELECT qcm.avg_quantity_campaign, qnc.avg_non_quantity_campaign
FROM quantity_during_campaign qcm, quantity_during_non_campaign qnc
...
SELECT cr.totalrevenue_campaign AS total_revenue_during_campaign,
ncr.totalrevenue_non_campaign AS total_revenue_during_non_campaign
FROM  campaign_revenue cr ,non_campaign_revenue ncr

--10. Rank the products by their average daily quantity sold

with cte1 AS (
SELECT sc.product_name, ROUND(AVG(t.quantity),2) AS avg_daily_quantity
FROM transactions t
JOIN sustainable_clothing sc
ON t.product_id = sc.product_id
GROUP BY sc.product_name)

SELECT cte1.*, DENSE_RANK() OVER(ORDER BY avg_daily_quantity DESC) AS rnk
FROM cte1
ORDER BY rnk